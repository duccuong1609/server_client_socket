...
@NamedQueries({
	...
	@NamedQuery(name = "Student.findStudentsEnrolledCourse", 
			query = "select s from Student s inner join s.studentGrades sg where sg.course.id in (select c.id from Course c where c.title like :title)")
})
